...
phase แรกจำกัดรายการ tool ให้เหลือ `plan_write` แต่ไม่ใช้ `tool_choice=required` ซึ่ง
Alibaba Qwen thinking mode ปฏิเสธด้วย HTTP 400
...
- LLM ประกาศ `required_capability` และ `evidence_requirements` แบบ typed ตอน
  `plan_write/plan_revise`; Python ไม่เดา step intent จาก keyword ใน description
- Dynamic Observation Policy ตรวจ typed step + action capability + result type
...
```text
LLM proposes action
→ LLM writes typed capability/evidence intent
→ Python validates state + typed plan + analytical contract
→ MCP executes
...
```text
Tool result
→ read required capability from typed PlanStep (ไม่อ่าน keyword จาก description)
→ infer structural action capabilities (รวม SQL ที่อ่าน INFORMATION_SCHEMA)
→ load resolved declarative contract
→ evaluate model-declared evidence predicates + compile typed claims
→ ObservationState
...
Stability baseline ก่อน typed-step refactor จากคำถามเดิม 10 runs: completion 9/10,
strict semantic pass 5/10, pass with caveat 2/10, false accept 2/10,
safe max-step failure 1/10 และ median tool calls ของรอบที่จบเท่ากับ 6
