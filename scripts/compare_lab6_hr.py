...
<style>*{{box-sizing:border-box}}body{{margin:0;background:#090e1c;color:#edf1ff;font:16px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace}}main{{max-width:1180px;margin:28px auto;padding:0 24px}}h1{{color:#8be9fd;font-size:34px;margin:0}}.sub{{color:#9ca8c7;margin:7px 0 22px}}.grid{{display:grid;grid-template-columns:1fr 1fr;gap:18px}}.card,.proof{{background:#111a31;border:1px solid #2c3a60;border-radius:14px;padding:20px}}.old{{border-top:5px solid #ffb86c}}.new{{border-top:5px solid #50fa7b}}h2{{margin:0 0 15px}}.metrics{{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}}.metric{{background:#182440;border-radius:9px;padding:14px;text-align:center;color:#aeb8d4}}.metric b{{display:block;font-size:25px;color:#50fa7b}}.old .metric b{{color:#ffb86c}}.proof{{margin-top:16px;border-left:5px solid #bd93f9}}.proof b{{display:block;color:#bd93f9}}code{{color:#f1fa8c}}</style><main>
...
def main():
    ...
    todo, todo_calls, todo_ms = execute(
        "TODO ORIGINAL", lambda q, r: run_todo(q, r, return_details=True), challenge["question"]
    )
    planner, planner_calls, planner_ms = execute("PURE PLANNER", run_planner, challenge["question"])
    ...
    prefix = Path(f"artifacts/lab6_hr_comparison_{args.challenge}")
